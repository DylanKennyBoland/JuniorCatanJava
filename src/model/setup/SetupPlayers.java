...
public class SetupPlayers {
	...
	public void CreateAllPlayers(Scanner player) {
		...
		this.stockpile.update("Wood", -playerList.getNumOfPlayers());
		this.stockpile.update("Molasses", -playerList.getNumOfPlayers());
	}
	...
	private String getPlayerColour(Scanner player) {
		...
		while (!validColour) {
			...
			switch (choice) {
			case "1":
				validColour = isValidColour(availableColors.indexOf(PlayerEnums.BLUE));
				return "BLUE";
			case "2":
				validColour = isValidColour(availableColors.indexOf(PlayerEnums.RED));
				return "RED";
			case "3":
				validColour = isValidColour(availableColors.indexOf(PlayerEnums.ORANGE));
				return "ORANGE";
			case "4":
				validColour = isValidColour(availableColors.indexOf(PlayerEnums.WHITE));
				return "WHITE";
			}
		}
		return " ";
